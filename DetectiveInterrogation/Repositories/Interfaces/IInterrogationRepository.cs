...
public interface IInterrogationRepository
{
    ...
    Task<SuspectReply?> GetReplyAsync(int suspectId, int phraseId, int trust, int pressure);
    Task<bool> IsEvidenceUsedAsync(int sessionId, int evidenceId);
    Task<List<int>> GetUsedEvidenceIdsAsync(int sessionId);
    Task<List<AvailablePhraseDto>> GetAvailablePhrasesAsync(int suspectId, List<int> usedEvidenceIds);
